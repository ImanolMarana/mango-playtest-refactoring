...
final class MessageFormatter {
  ...
  for (int L = 0; L < argArray.length; L++) {
    ...
    if (j == -1) {
      ...
      handleEscapedDelimeter(messagePattern, argArray, sbuf, L, j);
      i = j + 2;
    }
  }
  // append the characters following the last {} pair.
  sbuf.append(messagePattern.substring(i, messagePattern.length()));
  return createFormattingTuple(argArray, sbuf, throwableCandidate);
}

private static FormattingTuple createFormattingTuple(final Object[] argArray, StringBuffer sbuf,
    Throwable throwableCandidate) {
  if (argArray.length > 0 && argArray[argArray.length - 1] instanceof Throwable) {
...
private static void handleEscapedDelimeter(final String messagePattern, final Object[] argArray,
    StringBuffer sbuf, int L, int j) {
  if (isEscapedDelimeter(messagePattern, j)) {
    if (!isDoubleEscaped(messagePattern, j)) {
      L--; // DELIM_START was escaped, thus should not be incremented
      sbuf.append(messagePattern.substring(j - 1, j));
    } else {
      // The escape character preceding the delimiter start is
      // itself escaped: "abc x:\\{}"
      // we have to consume one backward slash
      sbuf.append(messagePattern.substring(j - 2, j));
